...
def run_tick(snapshot: list[dict], policy: Policy, draft=None, announce=None) -> TickResult:
    ...
    if d.act:
        ...
        status_se = action.status_announce(snapshot, safety_r.failsafe, announce)
    ...
    side_effects = action.status_announce(snapshot, safety_r.failsafe, announce)
